import React, { useEffect, useState, useCallback } from "react";
import { Link } from "react-router-dom";
import {
  FaTachometerAlt,
  FaListUl,
  FaUsers,
  FaPaperPlane,
  FaPlus,
  FaArrowRight,
  FaSyncAlt,
  FaFileAlt,
} from "react-icons/fa";

const API_URL = "http://localhost:5000";

// ------------------------------------------------------------------
// HELPER DARI HasilUjian.jsx
// Diperlukan untuk menghitung total peserta unik dari /api/hasil
// ------------------------------------------------------------------
const groupDataByPeserta = (data) => {
  const grouped = data.reduce((acc, row) => {
    const id = row.peserta_id;
    if (!acc[id]) {
      acc[id] = {
        peserta_id: id,
        nama: row.nama,
        pg_benar: 0,
        total_pg: 0,
      };
    }
    if (row.tipe_soal === "pilihanGanda") {
      acc[id].total_pg += 1;
      if (row.benar) {
        acc[id].pg_benar += 1;
      }
    }
    return acc;
  }, {});

  return Object.values(grouped).map((peserta) => {
    const skor_pg =
      peserta.total_pg > 0
        ? ((peserta.pg_benar / peserta.total_pg) * 100).toFixed(0)
        : 0;
    return {
      ...peserta,
      skor_pg: skor_pg,
    };
  });
};

// ------------------------------------------------------------------
// HELPER DARI DaftarSoal.jsx
// Diperlukan untuk memformat tanggal ujian
// ------------------------------------------------------------------
const pad2 = (n) => String(n).padStart(2, "0");
const toLocalDateOnly = (val) => {
  const d = new Date(val);
  if (!isNaN(d)) {
    return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())}`;
  }
  const s = String(val || "");
  return s.includes("T") ? s.slice(0, 10) : s;
};
const formatTanggal = (tgl) => {
  if (!tgl) return "-";
  const [y, m, d] = toLocalDateOnly(tgl).split("-");
  if (!y || !m || !d) return tgl;
  return `${d}-${m}-${y}`;
};

// ------------------------------------------------------------------
// KOMPONEN KARTU STATISTIK (Reusable)
// ------------------------------------------------------------------
const StatCard = ({ icon, label, value, to, color }) => (
  <Link
    to={to}
    className={`bg-white p-5 rounded-xl shadow-md border-l-4 ${color} transition-all hover:shadow-lg hover:scale-[1.02]`}
  >
    <div className="flex items-center justify-between">
      <div className="flex items-center">
        <div className="p-3 rounded-full bg-gray-100">{icon}</div>
        <div className="ml-4">
          <p className="text-sm font-medium text-gray-500">{label}</p>
          <p className="text-3xl font-bold text-gray-900">{value}</p>
        </div>
      </div>
      <FaArrowRight className="text-gray-400" />
    </div>
  </Link>
);

// ------------------------------------------------------------------
// KOMPONEN UTAMA DASHBOARD
// ------------------------------------------------------------------
const DashboardAdmin = () => {
  const [stats, setStats] = useState({
    totalUjian: 0,
    totalPeserta: 0,
    totalUndangan: 0,
  });
  const [recentUjian, setRecentUjian] = useState([]);
  const [recentHasil, setRecentHasil] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const token = sessionStorage.getItem("adminToken");
      if (!token) {
        console.error("Token admin tidak ditemukan di sessionStorage.");
        return; 
      }
      const headers = { Authorization: `Bearer ${token}` }; // Siapkan header

      const [resUjian, resHasil, resUndangan] = await Promise.allSettled([
        fetch(`${API_URL}/api/ujian`, { headers }),       // <-- Tambah headers
        fetch(`${API_URL}/api/hasil`, { headers }),       // <-- Tambah headers
        fetch(`${API_URL}/api/invite/list`, { headers }), // <-- Tambah headers
      ]);

      let totalUjian = 0,
        totalPeserta = 0,
        totalUndangan = 0,
        listUjian = [],
        listHasil = [];

      // Proses data Ujian (dari DaftarSoal.jsx)
      if (resUjian.status === "fulfilled" && resUjian.value.ok) {
        const dataUjian = await resUjian.value.json();
        totalUjian = dataUjian.length;
        // Urutkan dari yang terbaru, ambil 5
        listUjian = [...dataUjian]
          .sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
          .slice(0, 5);
      }

      // Proses data Hasil (dari HasilUjian.jsx)
      if (resHasil.status === "fulfilled" && resHasil.value.ok) {
        const dataHasil = await resHasil.value.json();
        const groupedData = groupDataByPeserta(dataHasil);
        totalPeserta = groupedData.length;
        // Ambil 5 hasil "terbaru" (berdasarkan data terakhir di array)
        listHasil = groupedData.slice(-5).reverse();
      }

      // Proses data Undangan (dari DaftarUndangan.jsx)
      if (resUndangan.status === "fulfilled" && resUndangan.value.ok) {
        const dataUndangan = await resUndangan.value.json();
        totalUndangan = dataUndangan.length;
      }

      setStats({ totalUjian, totalPeserta, totalUndangan });
      setRecentUjian(listUjian);
      setRecentHasil(listHasil);
    } catch (err) {
      console.error("Gagal memuat data dashboard:", err);
      alert("Gagal memuat data dashboard.");
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    fetchData();
  }, [fetchData]);

  if (loading) {
    return (
      <div className="bg-gray-50 min-h-screen flex justify-center items-center">
        <FaSyncAlt className="animate-spin text-4xl text-blue-600" />
      </div>
    );
  }

  return (
    <div className="bg-gray-50 min-h-screen flex flex-col">
      {/* 1. HEADER */}
      <div className="bg-white shadow-sm border-b border-gray-300 px-8 py-5 sticky top-0 z-50">
        <h2 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
          <FaTachometerAlt className="text-blue-600 w-6 h-6" />
          Dashboard Admin
        </h2>
      </div>

      {/* 2. KONTEN */}
      <div className="p-6 md:p-10 flex-1">
        <div className="max-w-7xl mx-auto space-y-8">
          {/* 2.1 RINGKASAN STATISTIK */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatCard
              to="/admin/daftar-soal"
              label="Daftar Ujian Tersedia"
              value={stats.totalUjian}
              color="border-blue-500"
              icon={<FaListUl className="text-blue-500 text-2xl" />}
            />
            <StatCard
              to="/admin/hasil-ujian"
              label="Total Peserta Mengerjakan"
              value={stats.totalPeserta}
              color="border-green-500"
              icon={<FaUsers className="text-green-500 text-2xl" />}
            />
            <StatCard
              to="/admin/tambah-peserta"
              label="Total Undangan Terkirim"
              value={stats.totalUndangan}
              color="border-yellow-500"
              icon={<FaPaperPlane className="text-yellow-500 text-2xl" />}
            />
          </div>

          {/* 2.2 AKSI CEPAT */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link
              to="/admin/tambah-soal"
              className="bg-white p-6 rounded-xl shadow-md border border-gray-200 flex justify-between items-center transition-all hover:bg-gray-50 hover:shadow-lg"
            >
              <div>
                <h3 className="text-xl font-semibold text-gray-800">
                  Tambah Ujian
                </h3>
                <p className="text-gray-500 text-sm mt-1">
                  Mulai rancang soal pilihan ganda atau esai.
                </p>
              </div>
              <div className="p-3 bg-blue-100 rounded-full">
                <FaPlus className="text-blue-600 text-xl" />
              </div>
            </Link>
            <Link
              to="/admin/tambah-peserta"
              className="bg-white p-6 rounded-xl shadow-md border border-gray-200 flex justify-between items-center transition-all hover:bg-gray-50 hover:shadow-lg"
            >
              <div>
                <h3 className="text-xl font-semibold text-gray-800">
                  Undang Peserta
                </h3>
                <p className="text-gray-500 text-sm mt-1">
                  Kirim undangan ujian ke email peserta.
                </p>
              </div>
              <div className="p-3 bg-green-100 rounded-full">
                <FaPaperPlane className="text-green-600 text-xl" />
              </div>
            </Link>
          </div>

          {/* 2.3 AKTIVITAS TERBARU */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Ujian Terbaru */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
              <div className="flex justify-between items-center mb-4 border-b pb-3">
                <h3 className="text-lg font-semibold text-gray-800">
                  Ujian Terbaru
                </h3>
                <Link
                  to="/admin/daftar-soal"
                  className="text-sm font-medium text-blue-600 hover:underline"
                >
                  Lihat Semua
                </Link>
              </div>
              <div className="space-y-3">
                {recentUjian.length > 0 ? (
                  recentUjian.map((ujian) => (
                    <div
                      key={ujian.id}
                      className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200"
                    >
                      <div>
                        <p className="font-medium text-gray-700">
                          {ujian.keterangan}
                        </p>
                        <p className="text-xs text-gray-500">
                          Tanggal: {formatTanggal(ujian.tanggal)}
                        </p>
                      </div>
                      <Link
                        to={`/admin/edit-soal/${ujian.id}`}
                        className="text-blue-600 hover:text-blue-800"
                        title="Edit Soal"
                      >
                        <FaArrowRight />
                      </Link>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 text-center py-4">
                    Belum ada ujian yang dibuat.
                  </p>
                )}
              </div>
            </div>

            {/* Hasil Terbaru */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
              <div className="flex justify-between items-center mb-4 border-b pb-3">
                <h3 className="text-lg font-semibold text-gray-800">
                  Hasil Terbaru Peserta
                </h3>
                <Link
                  to="/admin/hasil-ujian"
                  className="text-sm font-medium text-blue-600 hover:underline"
                >
                  Lihat Semua
                </Link>
              </div>
              <div className="space-y-3">
                {recentHasil.length > 0 ? (
                  recentHasil.map((hasil) => (
                    <div
                      key={hasil.peserta_id}
                      className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200"
                    >
                      <div>
                        <p className="font-medium text-gray-700">
                          {hasil.nama}
                        </p>
                        <p className="text-xs text-gray-500">
                          Skor PG:{" "}
                          <span className="font-bold text-blue-700">
                            {hasil.skor_pg}%
                          </span>
                        </p>
                      </div>
                      <Link
                        to={`/admin/hasil/${hasil.peserta_id}`}
                        className="text-blue-600 hover:text-blue-800"
                        title="Lihat Detail"
                      >
                        <FaFileAlt />
                      </Link>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500 text-center py-4">
                    Belum ada peserta yang mengerjakan.
                  </p>
                )}
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default DashboardAdmin;